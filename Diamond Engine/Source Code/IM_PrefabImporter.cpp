...
int PrefabImporter::SavePrefab(const char* assets_path, GameObject* gameObject)
{
	...
	gameObject->SaveToJson(json_value_get_array(goArray), true);
...
GameObject* PrefabImporter::LoadUIPrefab(const char* libraryPath)
{
	...
	int sceneParentId = EngineExternal->moduleGui->GetCanvasId();
	GameObject* sceneParent = EngineExternal->moduleScene->GetGOFromUID(EngineExternal->moduleScene->root, sceneParentId);

	if (sceneParent == nullptr)
		sceneParent = EngineExternal->moduleScene->root;


	rootObject = new GameObject(json_object_get_string(goJsonObj, "name"), sceneParent, json_object_get_number(goJsonObj, "UID"));
	rootObject->LoadFromJson(goJsonObj);
