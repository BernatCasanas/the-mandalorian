...
public:
	...
	void RenderMesh(GLuint textureID, float3 color, bool renderTexture = false, ResourceMaterial* shader = nullptr, C_Transform* _transform = nullptr, ResourceTexture* normalMap = nullptr, ResourceTexture* specularMap = nullptr, float emissionAmmount = 0.0f);
	...
	void OGL_GPU_Render();
	void PushDefaultMeshUniforms(uint shaderID, uint textureID, C_Transform* _transform, float3 color, ResourceTexture* normalMap = nullptr, ResourceTexture* specularMap = nullptr, float emissionAmmount = 0.0f);
